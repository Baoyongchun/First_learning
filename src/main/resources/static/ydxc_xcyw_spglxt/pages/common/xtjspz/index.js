        ...
        methods: {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.code === "200") {
                            ...
                            if (_this.pzjsList.length > 0) {
                                _this.selectJueseType(_this.pzjsList[0].code, _this.pzjsList[0].name);
                            }
                        }
                    },
                    error: function (data,textStatus, errorThrown) {
            ...
            getQueryTypeList:function() {
                ...
                for(var i = 0;i<this.pzjsList.length;i++){
                    if(this.jglx==1){
                        if(this.pzjsList[i].name == '审核监督员' || this.pzjsList[i].name == '公章管理员'){
                            continue;
                        }
                        _dataList.push({
                            codeType:'10100003',
                        ...
                        if(this.pzjsList[i].name == '审核员'||this.pzjsList[i].name == '监督员' || this.pzjsList[i].name == '公章管理员'){
                            continue;
                        }
                        _dataList.push({
                            codeType:'10100003',
            ...
            confirmModal: function (){
                ...
                _this.saveAllRole()
                /*if (_this.canEdit) {
                    if(_this.jglx=='' || _this.jglx == _this.dwObject.jglx){
                        _this.corpConfig = null;
                        _this.saveAllRole()
                    } else {
                        if(_this.jglx==1){
                            _this.confirmMessage = '修改为四员角色后审核监督员角色会失效，是否确认修改？'
                        }else {
                            _this.confirmMessage = '修改为三员角色后审核员、监督员角色会失效，是否确认修改？'
                        }
            ...
            confirmChange: function () {
                const _this = this;
                this.jglx = this.confirmType;
                this.jueSeSelected = '';
                this.jueSeSelectedCode = '';
                this.getPzxx()
                _this.corpConfig = {
                ...
                //单位角色
                let corpConfig = _this.corpConfig;
                let _serverData = {
                    corpConfig: corpConfig,
                    code: '',
                }
                //只保存单位的角色，并清空审核监督员或者审核员、监督员角色
                $.ajax({
            ...
            switchType: function (val) {
                ...
                if (_this.canEdit) {
                    ...
                    _this.confirmType = val
                    _this.$refs.confirmModal.open();
                }
            },
            // 选择角色
            selectJueseType: function (code, name) {
                this.tableShow = true;
                this.jueSeSelected = name;
                this.jueSeSelectedCode = code;
                $(".fd-list-btn").removeClass("select")
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.code === '200') {
                            ...
                            _this.$refs.confirmsvModal.close();
                            _this.$refs.confirmModal.close();
                        } else {
                            $.alert({
                                type: 'fail',
            ...
            confirmModalsv:function(){
                var _this = this;
                _this.selecteNames = _this.$refs.multipleTable.selection.map(user => user.name);
                let userIds = _this.$refs.multipleTable.selection.map(user => user.id);
                let cleanUserids = _this.zzjgyhglDataList.filter(item => item.jueSeSelected && $.inArray(item.id, userIds) === -1).map(user => user.id)
                if( _this.selecteNames.length==0 && cleanUserids.length==0){
                    return ;
                }
                if(_this.jueSeSelected.indexOf("审核监督员")>-1 ||_this.jueSeSelected.indexOf("审核员")>-1){
                    _this.selecteNames = _this.selecteNames.toString();
                    _this.$refs.confirmsvModal.open();
                }else{
                    _this.confirmModal();
                }
            }
