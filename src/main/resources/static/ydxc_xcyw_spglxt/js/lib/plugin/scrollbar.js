	...
	$.fn.addScrollBar = function (options) {
		...
		_scope.scrollBar = {
			...
			mousewheel : function (event) {
				if($(event.currentTarget).find('.fd-ajxx-popup').length > 0){
					//隐藏ajxx弹窗
	           		$(".fd-ajxx-popup").css({"display":"none"});
	           		//隐藏选择状态
	           		$(".fd-sfxx-tool li").removeClass("select");
				}
			...
			mousedown : function (event) {
				...
				//  垂直滚动条
					var _start = _scope.opts.direction=='vertical' ? event.pageY:event.pageX,
						_moveDistance = 0,
						_startPageX=event.pageX,
						_end = 0;
						_that.addClass(_scope.opts.pressClass);
				$(document).on('mousemove.scrollBar', function (event) {
					var event=_scope.scrollBar.getEvent(event),
						_endPageX=event.pageX;
					//判断鼠标按下之后 如果拖动的距离大于自定义的距离 则 停止执行滚动事件
					if((_endPageX>_startPageX && _endPageX-_startPageX>5)||(_endPageX<_startPageX && _startPageX-_endPageX>50)){
