...
(function ($, window) {
	$.fn.addSelect = function (options) {
		...
		$(this).each(function () {
			...
			// 绑定下拉事件
			_triggerElement.unbind().bind(opts.mouseType, function (event) {
				//阻止事件冒泡
			...
			$(document).bind("click", function (event) {
				...
				//  隐藏元素
				searchContentContain.hide();
				searchContentContain.parent().css({"borderColor":" #d9d9d9"});
				searchContentContain.parent().find(".search_value_box_sub").addClass("search_value_box_sub_borderm");
				searchContentContain.parent().find(".search_value_box_sub").removeClass("search_value_box_sub_borderh");
			});
