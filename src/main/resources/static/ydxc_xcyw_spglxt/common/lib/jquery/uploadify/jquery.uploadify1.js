...
var swfobject = function() {
	var D = "undefined", r = "object", S = "Shockwave Flash", W = "ShockwaveFlash.ShockwaveFlash", q = "application/x-shockwave-flash", R = "SWFObjectExprInst", x = "onreadystatechange", O = window, j = document, t = navigator, T = false, U = [h], o = [], N = [], I = [], l, Q, E, B, J = false, a = false, n, G, m = true, M = function() {
		var aa = typeof j.getElementById != D
				...
				.toLowerCase(), ae = Y ? /win/.test(Y) : /win/.test(ah), ac = Y
				? /mac/.test(Y)
...
SWFUpload.prototype.initSettings = function() {
	...
	if (!this.settings.preserve_relative_urls) {
		...
		//TODO LIKUN 原来问题的症结在于SWFUpload，这个是Uploadify调用供支持Flash上传用的，但是其没有对button_image_url进行判断，导致如果button_image_url为空的话就默认请求了当前根路径，修改的方法也比较简单，加个判断即可：
...
(function($) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					overrideEvents : []
						 ...
						 * files in the queue have bean uploaded onSelectError //
				...
				if (flashInstalled) {
					...
					$button.html('<span class="uploadify-button-text">'
							+ settings.buttonText + '</span>').css({
								'height' : settings.height + 'px',
					...
					swfuploadify.queueData = {
						...
						queueBytesUploaded : 0, // The size in bytes that have
												// bean uploaded for the current
												// upload queue
						uploadQueue : [], // The files currently to be
											// uploaded
						errorMsg : '一些文件没有被添加到队列中：'
					};
			...
			this.each(function() {
				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						$('#' + settings.queueID).find('.uploadify-queue-item')
								.each(function() {
									delay++;
									if (args[1] === true) {
										swfuploadify.cancelUpload($(this)
														.attr('id'), false);
									} else {
										swfuploadify.cancelUpload($(this)
												.attr('id'));
									...
									$(this).find('.data').removeClass('data')
											.html(' - Cancelled');
									$(this).find('.uploadify-progress-bar')
											.remove();
									$(this).delay(1000 + 100 * delay).fadeOut(
											500, function() {
												$(this).remove();
											});
								});
						...
						for (var n = 0; n < args.length; n++) {
							...
							$('#' + args[n]).find('.data').removeClass('data')
									.html(' - Cancelled');
							$('#' + args[n]).find('.uploadify-progress-bar')
									.remove();
					...
					var item = $('#' + settings.queueID)
							.find('.uploadify-queue-item').get(0);
					$item = $(item);
				...
				if (args.length === 1) {
					...
					switch (name) {
						...
						case 'fileTypeExts' :
							...
							break;
						case 'fileTypeDesc' :
							swfuploadify.setFileTypes(settings.fileTypeExts,
									value);
							break;
						case 'fileSizeLimit' :
						...
						case 'buttonText' :
							$('#' + settings.id + '-button')
									.find('.uploadify-button-text').html(value);
							break;
						case 'width' :
							...
							break;
						case 'height' :
							swfuploadify.setButtonDimensions(settings.width,
									value);
							break;
						case 'multi' :
							if (value) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg = '一些文件没有被添加到队列中：';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('名为 “'
							+ file.name
							+ '”的文件已经在队列中。\n是否想要替换已存在的文件？');
					if (!replaceQueueItem) {
		...
		// Triggered when all the files in the queue have bean processed
		...
		onUploadComplete : function(file) {
			...
			//TODO LIKUN 默认将异常放在后台处理，此处假设都是正常即可（Uncaught Call to GetStats failed）
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			switch (errorCode) {
				...
				case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED :
					alert('The upload limit has bean reached (' + errorMsg
							+ ').');
